...
export default function AllMyIssues(){
  ...
  function handleDifficultyChange(idx, level){
    try{
      ...
      if(!item) return
      const id = item.id
      const payload = { ...item, difficulty: level, attachmentsJson: JSON.stringify(item.attachments || []) }
      fetch(`${API_BASE}/api/issues/${id}`, { method: 'PUT', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) })
        .then(res => { if(!res.ok) throw new Error('update failed'); return fetch(`${API_BASE}/api/issues`) })
        .then(r=>r.json())
        .then(data=>{
          const parsed = data.map(d => ({ ...d, attachments: d.attachmentsJson ? JSON.parse(d.attachmentsJson) : (d.attachments || []) }))
          setIssues(parsed.slice().reverse())
        })
        .catch(err=>{ console.error(err); alert('Failed to update difficulty') })
    }catch(e){ console.error(e) }
  }
              ...
              <div style={{marginTop:10, display:'flex', alignItems:'center', gap:12}}>
                ...
                <div className="difficulty-group">
                  <div className="difficulty-radio high">
                    <input id={`diff-${idx}-high`} type="radio" name={`difficulty-${idx}`} checked={it.difficulty === 'High'} onChange={()=>handleDifficultyChange(idx,'High')} />
                    <label htmlFor={`diff-${idx}-high`}><span className="dot"/>High</label>
                  </div>
                  <div className="difficulty-radio medium">
                    <input id={`diff-${idx}-medium`} type="radio" name={`difficulty-${idx}`} checked={!it.difficulty || it.difficulty === 'Medium'} onChange={()=>handleDifficultyChange(idx,'Medium')} />
                    <label htmlFor={`diff-${idx}-medium`}><span className="dot"/>Medium</label>
                  </div>
                  <div className="difficulty-radio low">
                    <input id={`diff-${idx}-low`} type="radio" name={`difficulty-${idx}`} checked={it.difficulty === 'Low'} onChange={()=>handleDifficultyChange(idx,'Low')} />
            ...
            <div className="create-issue-form">
              ...
              <div className="form-row">
                ...
                <div className="form-row">
                  ...
                  <div className="difficulty-group">
                    <div className="difficulty-radio high">
                      <input id="edit-diff-high" type="radio" name="edit-difficulty" checked={editFields.difficulty==='High'} onChange={()=>setEditFields(prev=>({...prev,difficulty:'High'}))} />
                      <label htmlFor="edit-diff-high"><span className="dot"/>High</label>
                    </div>
                    <div className="difficulty-radio medium">
                      <input id="edit-diff-medium" type="radio" name="edit-difficulty" checked={editFields.difficulty==='Medium'} onChange={()=>setEditFields(prev=>({...prev,difficulty:'Medium'}))} />
                      <label htmlFor="edit-diff-medium"><span className="dot"/>Medium</label>
                    </div>
                    <div className="difficulty-radio low">
                      <input id="edit-diff-low" type="radio" name="edit-difficulty" checked={editFields.difficulty==='Low'} onChange={()=>setEditFields(prev=>({...prev,difficulty:'Low'}))} />
